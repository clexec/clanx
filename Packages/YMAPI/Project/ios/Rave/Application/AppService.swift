...
class AppService
{
    ...
    init() {
        ...
        //let device = YMDevice.generateWebMimicDevice(uuid: uuid)
        ...
        if (properties.isAuthed) {
            client = YMClient.initialize(device: device, lang: lang, uid: properties.uid, token: AppService.getToken(), xToken: "")
        } else {
