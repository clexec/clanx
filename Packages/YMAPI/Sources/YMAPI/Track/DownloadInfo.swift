...
public class DownloadInfo: Decodable {
    ...
    public func getDirectLink(web: Bool = false, completion: @escaping (_ result: Result<String, YMError>) -> Void) {
        var downloadLinkRetriever = getTrackDownloadLinkByApi
        if (web) {
            downloadLinkRetriever = getTrackDownloadLinkByWebApi
        }
        ...
        downloadLinkRetriever(downloadInfoUrl) { result in
            do {
    ...
    public func downloadTrack(web: Bool = false, completion: @escaping (_ result: Result<Data, YMError>) -> Void) {
        if let g_directLink = directLink, linkAlive {
            download(fullPath: g_directLink, completion: completion)
        } else {
            getDirectLink(web: web){ result in
                do {
                    let link = try result.get()
