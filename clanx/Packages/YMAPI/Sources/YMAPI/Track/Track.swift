...
public class Track: YMBaseObject, Decodable {
    ...
    ///Gets track download info variants (codec, bitrate and etc) without trtack direct download links. Download info will be active for 2 minutes after first direct download link fetching
    public func getDownloadInfoWeb( completion: @escaping (_ result: Result<[DownloadInfo], YMError>) -> Void){
        getTrackDownloadInfoByWebApi(token: accountSecret, trackId: trackId) { result in
            do {
                let downloadInfos = try result.get()
                self.downloadInfo = downloadInfos
                completion(result)
    ...
    ///Gets track download info (version 2, with streaming support) variants (codec, bitrate and etc) without trtack direct download links. Download info will be active for 2 minutes after first direct download link fetching
    ///- Parameter canUseStreaming: Apply stream containers use flag
    public func getDownloadInfoV2Web(canUseStreaming: Bool, completion: @escaping (_ result: Result<[DownloadInfo], YMError>) -> Void){
        let timestamp = Int(Date().timeIntervalSince1970)
        
        getTrackDownloadInfoV2ByWebApi(token: accountSecret, trackId: trackId, canUseStreaming: canUseStreaming, timestamp: String(timestamp)) { result in
            do {
                let downloadInfos = try result.get()
                self.downloadInfo = downloadInfos
                completion(result)
    ...
    public func getDownloadLink(codec: TrackCodec = .mp3, bitrate: TrackBitrate = .kbps_192, web: Bool = false, completion: @escaping (_ result: Result<String, YMError>) -> Void) {
        var downloadInfoRetriever = getDownloadInfo
        if (web) {
            downloadInfoRetriever = getDownloadInfoWeb
        }
        if let g_downloadInfo = downloadInfo
        {
            if (g_downloadInfo.count == 0) {
                downloadInfoRetriever { result in
                    do {
                        let downloadInfos = try result.get()
                        if (downloadInfos.count > 0) {
                            self.getDownloadLink(codec: codec, bitrate: bitrate, web: web, completion: completion)
                        } else {
            ...
            for info in g_downloadInfo {
                if (info.codec == codec.rawValue && info.bitrateInKbps == bitrate.rawValue) {
                    info.getDirectLink(web: web) { result in
                        do {
    ...
    public func downloadTrack(codec: TrackCodec = .mp3, bitrate: TrackBitrate = .kbps_192, web: Bool = false, completion: @escaping (_ result: Result<Data, YMError>) -> Void) {
        var downloadInfoRetriever = getDownloadInfo
        if (web) {
            downloadInfoRetriever = getDownloadInfoWeb
        }
        if let g_downloadInfo = downloadInfo
        {
            if (g_downloadInfo.count == 0) {
                downloadInfoRetriever { result in
                    do {
                        let downloadInfos = try result.get()
                        if (downloadInfos.count > 0) {
                            self.downloadTrack(codec: codec, bitrate: bitrate, web: web, completion: completion)
                        } else {
                            var trackInfo = self.trackId
                            if let g_title = self.title {
                                trackInfo += " (" + g_title + ")"
            ...
            for info in g_downloadInfo {
                if (info.codec == codec.rawValue && info.bitrateInKbps == bitrate.rawValue) {
                    info.downloadTrack(web: web, completion: completion)
                    return
                }
            }
            let msg: String = "Codec " + codec.rawValue + " and bitrate " + bitrate.valueString + " not found"
