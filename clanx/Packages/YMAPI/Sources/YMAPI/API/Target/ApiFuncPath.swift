...
extension ApiFunction {
    ...
    var path: String {
        ...
        case .track_download_info_web(let trackId, _): return "tracks/" + trackId + "/download-info?from=web-main&external-domain=music.yandex.ru&overloading=false"
        ...
        case .track_download_info_v2_web(let trackId, let can_use_streaming, let timestamp, let sign, _): return "tracks/" + trackId + "/download-info?from=web-main&external-domain=music.yandex.ru&overloading=false&can_use_streaming=" + String(can_use_streaming) + "&ts=" + timestamp + "&sign=" + sign
