...
func getTrackDownloadLinkByWebApi(downloadInfoUrl: String, completion: @escaping (_ result: Result<String, YMError>) -> Void)
{
    let webDownloadInfoUrl = downloadInfoUrl.replacingOccurrences(of: "service-10-track", with: "web-main")
    guard let url: URL = URL(string: webDownloadInfoUrl) else {completion(.failure(.badRequest(errCode: -1, description: "Incorrect track download info url"))); return}
    var urlReq: URLRequest = URLRequest(url: url)
    urlReq.httpMethod = "GET"
    requestData(urlReq) { result in
        do
...
func getTrackDownloadInfoByWebApi(token: String, trackId: String, completion: @escaping (_ result: Result<[DownloadInfo], YMError>) -> Void)
{
    guard let req: URLRequest = buildWebRequest(for: .track_download_info_web(trackId: trackId, secret: token)) else {completion(.failure(.badRequest(errCode: -1, description: "Unable to build track donwload info request"))); return}
    requestYMResponse(req) { result in
        var response: YMResponse?
        do {
            let ymResponse = try result.get()
...
func getTrackDownloadInfoV2ByWebApi(token: String, trackId: String, canUseStreaming: Bool, timestamp: String, completion: @escaping (_ result: Result<[DownloadInfo], YMError>) -> Void)
{
    guard let sign = CryptoUtil.generateSign(trackId: trackId, timestamp: timestamp) else {completion(.failure(.badRequest(errCode: -1, description: "Unable create sign for track download info request"))); return}
                        
    guard let req: URLRequest = buildWebRequest(for: .track_download_info_v2_web(trackId: trackId, canUseStreaming: canUseStreaming, timestamp: timestamp, sign: sign, secret: token)) else {completion(.failure(.badRequest(errCode: -1, description: "Unable to build track donwload info request"))); return}
    requestYMResponse(req) { result in
        var response: YMResponse?
        do {
            let ymResponse = try result.get()
